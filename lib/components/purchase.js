...
export default function Purchase({ character, eddies }) {
  const [openModal, setOpenModal] = useState(false)
  const [count, setCount] = useState(0)

  return (
    <>
      <a className={styles.button} onClick={() => handlePurchase(setOpenModal, setCount, character, eddies)} >{'Buy'}</a>
      <EddiesPopup  isOpen={openModal} setOpen={setOpenModal} count={count} />
    </>
  )
}

async function handlePurchase(setOpenModal, setCount, character, eddies) {
