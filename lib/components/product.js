...
export default function Product({ product = null, reload = null }) {
  const [openModal, setOpenModal] = useState(false)

    return (
      <>
        <a className={styles.button} onClick={() => setOpenModal(true)} >{product ? 'Edit Product' : 'New Product'}</a>
        <EditProduct isOpen={openModal} setOpen={setOpenModal} product={product} reload={reload}/>
      </>
    )
}

function EditProduct({isOpen, setOpen, product = null, reload = null}) {

  const [categories, setCategories] = useState(null)
  const [corps, setCorps] = useState(null)

  useEffect(() => {
    setDropdown(getCategories, setCategories)
    setDropdown(getCorps, setCorps)
  }, [setCategories, setCorps])
