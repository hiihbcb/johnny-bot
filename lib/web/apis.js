...
export async function setProduct(product) {
    ...
    let response = await fetch(process.env.NEXT_PUBLIC_URL + 'api/auth/products', options)
    return await response.json()
}
...
export async function setEddies(playerId, eddies) {
    let body = JSON.stringify({
        method: "setEddies",
        playerId: playerId,
        eddies: eddies
    ...
    let response = await fetch(process.env.NEXT_PUBLIC_URL + 'api/auth/characters', options)
    return await response.json()
}
